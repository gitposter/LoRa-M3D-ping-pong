 ...
 ******************************************************************************
 * @file    drv/sx1276.c
 * @module  sx1276 module
 * @author  Mohamed Hnezli
 * @version V0.2
 * @date    12-04-2019
 * @brief   SX1276 transceiver driver.
 ******************************************************************************
 * @attention
 *
...
#include "stm32f3xx.h"                 // low level drivers
#include "drv/clk.h"                   // clk_delay
//#include "board.h"
 ...
 */
static void sx1276_spi_conf (void)
{
  /* SPI1 parameter configuration*/
  spi_.Instance = CONF_SPI_SX1276;
  ...
  spi_.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_4;
  ...
  spi_.Init.CRCPolynomial = 7;
  spi_.Init.CRCLength = SPI_CRC_LENGTH_DATASIZE;
  spi_.Init.NSSPMode = SPI_NSS_PULSE_ENABLE;
...
void
sx1276_gpio_conf (void)
{
  GPIO_InitTypeDef gpio;

  /* configure DIO0 pin */
  gpio.Pin = CONF_PIN_SX1276_DIO0;
  gpio.Mode = GPIO_MODE_INPUT;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init (CONF_GPIO_SX1276_DIO0, &gpio);

  /* configure RST pin */
  gpio.Pin = CONF_PIN_SX1276_RST;
  gpio.Mode = GPIO_MODE_OUTPUT_PP;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init (CONF_GPIO_SX1276_RST, &gpio);

  /* configure NSS pin */
  gpio.Pin = CONF_PIN_SX1276_NSS;
  gpio.Mode = GPIO_MODE_OUTPUT_PP;
  gpio.Pull = GPIO_NOPULL;
  gpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init (CONF_GPIO_SX1276_SPI, &gpio);

  /* configure SPI pin */
  gpio.Pin = CONF_PIN_SX1276_SCK | CONF_PIN_SX1276_MOSI | CONF_PIN_SX1276_MISO;
...
void SX1276Init( RadioEvents_t *events )
{
  ...
  for (idx = 0; idx < sizeof(RadioRegsInit) / sizeof(RadioRegisters_t); idx++)
    {
      SX1276SetModem( RadioRegsInit[idx].Modem );
      SX1276Write (RadioRegsInit[idx].Addr, RadioRegsInit[idx].Value);
    }
...
{
  // Set RESET pin to 0
  HAL_GPIO_WritePin(CONF_GPIO_SX1276_RST, CONF_PIN_SX1276_RST, GPIO_PIN_RESET);

  // Wait 1 ms
  clk_delay (1);

  // Configure RESET as input
  HAL_GPIO_WritePin(CONF_GPIO_SX1276_RST, CONF_PIN_SX1276_RST, GPIO_PIN_SET);
...
void SX1276WriteBuffer (uint8_t addr, uint8_t *buffer, uint8_t size)
{
  ...
  //NSS = 0;
  HAL_GPIO_WritePin(CONF_GPIO_SX1276_SPI, CONF_PIN_SX1276_NSS, GPIO_PIN_RESET);

  txdata = addr | 0x80;
  HAL_SPI_TransmitReceive(&spi_, &txdata, &rxdata, 1, 50);
  for (cursor = 0; cursor < size; cursor++)
    {
      txdata = buffer[cursor];
      HAL_SPI_TransmitReceive(&spi_, &txdata, &rxdata, 1, 50);
    }
...
void SX1276ReadBuffer (uint8_t addr, uint8_t *buffer, uint8_t size)
{
  ...
  for (cursor = 0; cursor < size; cursor++)
    {
      HAL_SPI_TransmitReceive(&spi_, &txdata, (buffer+cursor), 1, 50);
    }
